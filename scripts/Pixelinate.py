...
def pixel_detect(image: Image):
    ...
    return kCentroid(image, image.width, image.height, 2), np.median(hspacing), np.median(vspacing)
...
def pixelinate(img_path, value):
    ...
    if True:
        ...
        best_k = determine_best_k(downscale, max_k=128)
